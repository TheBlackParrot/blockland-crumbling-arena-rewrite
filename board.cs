...
function generateCABoard() {
	...
	if(%spaced) {
		%sp_z = ((%height-1)*(%brickdata.BrickSizeZ/5) + 12 + 25);
	}
	...
	for(%x=0;%x<%width;%x++) {
		for(%y=0;%y<%length;%y++) {
			for(%z=0;%z<%height;%z++) {
				if(%spaced) {
					%pos = %x*(%brickdata.BrickSizeX/2) SPC %y*(%brickdata.BrickSizeY/2) SPC (%z*(%brickdata.BrickSizeZ/5) + 10 + 25);
				} else {
